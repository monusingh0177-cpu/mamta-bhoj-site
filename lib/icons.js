...
// Small line-art marks for the hero's Wheat -> Mill -> Flour -> Food journey
// badges. Kept separate from journeyIcons (used by the 6-step milling
// section) even though wheat/mill overlap, so each set can evolve on its
// own without cross-affecting the other.
const heroFlowIcons = {
  wheat: `<svg viewBox="0 0 40 40" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M20 34V16"/><path d="M20 6c-5 3-5 8 0 13 5-5 5-10 0-13z"/><path d="M20 14c-4 2-4 6 0 10 4-4 4-8 0-10z"/></svg>`,
  mill: `<svg viewBox="0 0 40 40" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="20" cy="20" r="15"/><circle cx="20" cy="20" r="4"/><path d="M20 8v4M20 28v4M8 20h4M28 20h4M11 11l3 3M26 26l3 3M29 11l-3 3M14 26l-3 3"/></svg>`,
  flour: `<svg viewBox="0 0 40 40" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M7 32h26"/><path d="M11 32c0-9.5 4-17 9-17s9 7.5 9 17"/><path d="M16 32c.4-5.5 2-10 4-10s3.6 4.5 4 10"/></svg>`,
  food: `<svg viewBox="0 0 40 40" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="14" cy="11" r="4"/><circle cx="26" cy="11" r="4"/><path d="M6 32c0-6.5 3.6-11 8-11s8 4.5 8 11"/><path d="M18 32c0-6.5 3.6-11 8-11s8 4.5 8 11"/></svg>`,
};

// Hero carousel slide backgrounds. All four share the same "photo card"
// technique as heroMillVisual (gradient panel + layered gradient-shaded
// shapes) so the set feels cohesive; each variant's content points at a
// different stage of the wheat -> flour -> kitchen journey. Original
// artwork only — not styled or captioned as an actual photograph of any
// real facility.
function heroSlideArt(kind) {
  const uid = 'hs' + kind;
  const defs = `<defs>
      <linearGradient id="${uid}Panel" x1="0" y1="0" x2="1" y2="1">
        <stop offset="0%" stop-color="#0d2a5c"/>
        <stop offset="55%" stop-color="#15408c"/>
        <stop offset="100%" stop-color="#0a1c40"/>
      </linearGradient>
      <linearGradient id="${uid}PanelLight" x1="0" y1="0" x2="1" y2="1">
        <stop offset="0%" stop-color="#f7f4ec"/>
        <stop offset="100%" stop-color="#e8e0cc"/>
      </linearGradient>
      <linearGradient id="${uid}Kernel" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#fff3d6"/>
        <stop offset="35%" stop-color="#f0bd54"/>
        <stop offset="100%" stop-color="#a9761f"/>
      </linearGradient>
      <radialGradient id="${uid}Glow" cx="60%" cy="30%" r="60%">
        <stop offset="0%" stop-color="#f2c463" stop-opacity=".28"/>
        <stop offset="60%" stop-color="#f2c463" stop-opacity="0"/>
      </radialGradient>
      <filter id="${uid}Blur"><feGaussianBlur stdDeviation="4"/></filter>
    </defs>`;

  const wheatCluster = (x, y, scale) => `<g transform="translate(${x},${y}) scale(${scale})">
    ${millWheatEar(`url(#${uid}Kernel)`, 5)}
  </g>`;

  let content = '';
  if (kind === 'milling') {
    content = `
      <rect width="800" height="500" fill="url(#${uid}Panel)"/>
      <rect width="800" height="500" fill="url(#${uid}Glow)"/>
      <g filter="url(#${uid}Blur)" opacity=".5">
        <rect x="560" y="90" width="52" height="260" rx="8" fill="#cfe0f5"/>
        <rect x="630" y="70" width="52" height="280" rx="8" fill="#cfe0f5"/>
        <rect x="700" y="110" width="52" height="240" rx="8" fill="#cfe0f5"/>
        <path d="M560 90h192M560 350h192" stroke="#cfe0f5" stroke-width="4"/>
      </g>
      <g stroke="rgba(255,255,255,.18)" stroke-width="1">${Array.from({ length: 6 })
        .map((_, i) => `<line x1="0" y1="${70 + i * 70}" x2="800" y2="${70 + i * 70}"/>`)
        .join('')}</g>
      ${wheatCluster(120, 470, 0.8)}
      ${wheatCluster(210, 475, 0.65)}
      <ellipse cx="170" cy="452" rx="150" ry="16" fill="#040e26" opacity=".45"/>
    `;
  } else if (kind === 'grain') {
    content = `
      <rect width="800" height="500" fill="url(#${uid}Panel)"/>
      <rect width="800" height="500" fill="url(#${uid}Glow)"/>
      <circle cx="600" cy="230" r="190" fill="#f7f4ec" opacity=".06"/>
      <circle cx="600" cy="230" r="130" fill="#f7f4ec" opacity=".08"/>
      <g fill="#f7f4ec" opacity=".5">${Array.from({ length: 40 })
        .map((_, i) => `<circle cx="${480 + ((i * 53) % 260)}" cy="${110 + ((i * 37) % 240)}" r="${1.4 + (i % 3)}"/>`)
        .join('')}</g>
      ${wheatCluster(150, 472, 0.82)}
      ${wheatCluster(240, 478, 0.65)}
      ${wheatCluster(70, 478, 0.6)}
      <ellipse cx="180" cy="452" rx="170" ry="16" fill="#040e26" opacity=".45"/>
      <g stroke="var(--wheat-gold)" stroke-width="2" fill="none" opacity=".8"><path d="M330 260h140" stroke-dasharray="4 6"/><path d="M456 250l14 10-14 10"/></g>
    `;
  } else if (kind === 'quality') {
    content = `
      <rect width="800" height="500" fill="url(#${uid}PanelLight)"/>
      <rect x="0" y="0" width="800" height="500" fill="none" stroke="#0d2a5c" stroke-width="10"/>
      <g stroke="#0d2a5c" stroke-width="1" opacity=".08">${Array.from({ length: 9 })
        .map((_, i) => `<line x1="${i * 100}" y1="0" x2="${i * 100}" y2="500"/>`)
        .join('')}</g>
      <circle cx="400" cy="230" r="110" fill="none" stroke="#0d2a5c" stroke-width="6"/>
      <path d="M350 232l35 35 65-75" fill="none" stroke="var(--wheat-gold)" stroke-width="10" stroke-linecap="round" stroke-linejoin="round"/>
      <g fill="#0d2a5c" opacity=".85">
        <rect x="230" y="360" width="90" height="70" rx="6"/>
        <rect x="480" y="360" width="90" height="70" rx="6"/>
      </g>
      <g stroke="#fff" stroke-width="2.5" opacity=".9"><path d="M245 380h60M245 395h60M245 410h40"/><path d="M495 380h60M495 395h60M495 410h40"/></g>
      ${wheatCluster(690, 90, 0.75)}
    `;
  } else {
    content = `
      <rect width="800" height="500" fill="url(#${uid}Panel)"/>
      <rect width="800" height="500" fill="url(#${uid}Glow)"/>
      <ellipse cx="400" cy="300" rx="190" ry="60" fill="#f7f4ec" opacity=".95"/>
      <ellipse cx="400" cy="288" rx="170" ry="50" fill="#fffdf6"/>
      <g fill="#f0bd54" opacity=".55">${Array.from({ length: 26 })
        .map((_, i) => `<circle cx="${260 + ((i * 31) % 280)}" cy="${268 + ((i * 17) % 40)}" r="${1.6 + (i % 2)}"/>`)
        .join('')}</g>
      <path d="M230 300c0 40 76 72 170 72s170-32 170-72" fill="none" stroke="#c9a25a" stroke-width="6" opacity=".6"/>
      ${wheatCluster(120, 472, 0.7)}
      ${wheatCluster(600, 440, 1.1)}
      <ellipse cx="400" cy="452" rx="220" ry="16" fill="#040e26" opacity=".3"/>
    `;
  }

  return `<svg viewBox="0 0 800 500" xmlns="http://www.w3.org/2000/svg" role="img" aria-hidden="true" preserveAspectRatio="xMidYMid slice">${defs}${content}</svg>`;
}
