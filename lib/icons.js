...
function heroField() {
  return `<svg viewBox="0 0 1200 500" preserveAspectRatio="xMidYMax slice" xmlns="http://www.w3.org/2000/svg" role="img" aria-hidden="true" class="hero-field-svg">
    <g opacity=".55">
      ${Array.from({ length: 34 })
        .map((_, i) => {
          const x = (i * 1200) / 33 + (i % 2 ? 10 : -6);
          const h = 150 + ((i * 37) % 90);
          const tilt = (i % 5) - 2;
          return `<g transform="translate(${x.toFixed(0)},500) rotate(${tilt})">
            <path d="M0 0 V-${h}" stroke="var(--wheat-gold)" stroke-width="2" stroke-linecap="round" opacity=".7"/>
            <path d="M0 -${h + 4} C -8 -${h - 4} -8 -${h - 16} 0 -${h - 24} C 8 -${h - 16} 8 -${h - 4} 0 -${h + 4} Z" fill="var(--wheat-gold)"/>
            <path d="M0 -${h - 8} C -7 -${h - 16} -7 -${h - 26} 0 -${h - 34} C 7 -${h - 26} 7 -${h - 16} 0 -${h - 8} Z" fill="var(--wheat-gold)" opacity=".85"/>
          </g>`;
        })
        .join('')}
    </g>
    <ellipse cx="600" cy="500" rx="700" ry="60" fill="var(--brand-navy-dark)" opacity=".5"/>
  </svg>`;
}
