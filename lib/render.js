...
function nav(active) {
  ...
  return `<header>
    <nav class="nav">
      <a href="/" class="brand">
        ${logoMark(38)}
        <span><span class="brand-name">Mamta Bhoj</span><br/><span class="brand-sub">Devmam Flourish Foods LLP</span></span>
      </a>
...
function whyGrid(icons, heading, sub) {
  return `<section class="wrap"><div class="section-head"><span class="eyebrow">Why Mamta Bhoj</span><h2>${escapeHtml(heading)}</h2>${sub ? `<p>${escapeHtml(sub)}</p>` : ''}</div>
    <div class="why-grid">
      <div class="why-card">${icons.trust.iso}<h4>Naturally Stone-Ground</h4><p>Chakki-milled to preserve fibre and nutrients.</p></div>
      <div class="why-card">${icons.trust.fssai}<h4>ISO &amp; FSSAI Compliant</h4><p>Milled and packed to ISO 9001:2015 and FSSAI standards.</p></div>
      <div class="why-card">${icons.trust.clock}<h4>Milled Fresh, Regularly</h4><p>Small, frequent milling batches.</p></div>
      <div class="why-card">${icons.trust.leaf}<h4>No Additives</h4><p>100% natural grain.</p></div>
    </div></section>`;
}

function promoStrip(icons) {
  return `<section class="wrap"><div class="promo-strip">
    <a href="/quality" class="promo-card">${icons.trust.stone}<div><strong>100% Natural &amp; Stone-Ground</strong><span>No additives, no shortcuts — every batch is chakki-ground the traditional way.</span></div></a>
    <a href="/quality" class="promo-card">${icons.trust.iso}<div><strong>ISO 9001:2015 &amp; FSSAI Certified</strong><span>Milled and packed under certified quality and food-safety standards.</span></div></a>
    <a href="/contact" class="promo-card">${icons.trust.leaf}<div><strong>Dealership &amp; Bulk Enquiries</strong><span>Looking to stock Mamta Bhoj or place a bulk order? We would love to hear from you.</span></div></a>
  </div></section>`;
}

function ctaBand(content) {
  const phoneHref = (content.phone || '').replace(/\s+/g, '');
  return `<section class="cta-band"><div class="wrap"><h2>For Dealership &amp; Bulk Enquiries</h2>
...
module.exports = { layout, nav, footer, logoMark, certBadge, whyGrid, promoStrip, ctaBand, escapeHtml, FONT_LINK };
